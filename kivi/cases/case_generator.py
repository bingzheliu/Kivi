...
def append_json_template(json_config, cur_json_config):
	...
	for d in cur_json_config["userDefined"]["dTypes"]:
		if d["podTemplateId"] in pt_map:
			d["podTemplateId"] = pt_map[d["podTemplateId"]]

	d_map = {}
	for t in ["nodesTypes", "dTypes"]:
		for i in range(0, cur_json_config["userDefined"][t]):
			n = cur_json_config["userDefined"][t][i]
			new_flag = True
			for exist_n in json_config["setup"][t]:
				if compare_template(n, exist_n, equal_templates[t]):
					new_flag = False
			...
			if new_flag:
				json_config["userDefined"][t].append(deepcopy(n))
				if t == "dTypes":
					d_map[i+1] = len(json_config["userDefined"][t])
...
def case_generator(case_id, scale, from_template=False, filename=None):
	...
	if case_id in case_fun:
		...
		for c in case_id:
			cur_json_config = generate_case_json(case_id, scale, from_template, filename)
